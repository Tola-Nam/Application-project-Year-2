  ...
  <div class="min-h-screen p-4 bg-gray-50">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="mb-6">
        <div class="flex items-center space-x-2 mb-2">
          ...
          <h1 class="text-2xl font-bold text-gray-900 capitalize">
            {{ route.query.category?.replace(/([A-Z])/g, ' $1').trim() || 'Products' }}
          </h1>
        </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-3">
        <div v-for="item in items" :key="item.pro_id"
          ...
          <router-link :to="{ path: 'detail', query: { category: item.category, id: item.pro_id } }" class="block">
            <!-- Compact Image -->
            <div class="relative">
              ...
              >
                <HeartIcon 
                  ...
                  :class="item.isFavorite ? 'text-red-500 fill-current' : 'text-gray-400 hover:text-red-400'"
                />
              </button>
            ...
            <div class="p-3 space-y-2">
              <!-- Product Name -->
              <h3 class="text-sm font-medium text-gray-900 line-clamp-2 leading-tight" :title="item.productName">
                {{ item.productName }}
              </h3>
              ...
              <div class="flex items-center justify-between text-xs text-gray-500">
                <div class="flex items-center space-x-2">
                  ...
                  <div class="flex items-center space-x-1" v-if="item.color">
                    <div class="w-3 h-3 rounded-full border border-gray-300" :style="{ backgroundColor: getColorCode(item.color) }"></div>
              ...
              <!-- Description -->
              <p class="text-xs text-gray-500 line-clamp-2 leading-relaxed" v-if="item.description">
                {{ item.description }}
              </p>
            </div>
          </router-link>
          ...
          <div class="px-3 pb-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
      ...
      <!-- Loading State -->
      <div v-if="loading" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-3">
        <div v-for="n in 12" :key="n" class="bg-white rounded-lg overflow-hidden shadow-sm border border-gray-100">
    ...
    <!-- Quick View Modal -->
    <div v-if="showQuickView" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" @click.self="showQuickView = false">
      <div class="bg-white rounded-xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <!-- Modal Header -->
        <div class="flex items-center justify-between p-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-900">Quick View</h3>
          <button @click="showQuickView = false" class="text-gray-400 hover:text-gray-600 transition-colors">
            <XIcon class="w-6 h-6" />
          </button>
        </div>
        ...
        <div v-if="selectedItem" class="p-4 space-y-4">
          ...
          <!-- Product Info -->
          <div class="space-y-3">
            <h4 class="text-xl font-bold text-gray-900">{{ selectedItem.productName }}</h4>
            <p class="text-2xl font-bold text-green-600">${{ parseFloat(selectedItem.price).toFixed(2) }}</p>
            
            <div class="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span class="text-gray-500">Stock:</span>
                <span class="ml-2 font-medium" :class="selectedItem.stock > 0 ? 'text-green-600' : 'text-red-600'">
            ...
            <div v-if="selectedItem.description">
              ...
              <p class="text-gray-600 text-sm leading-relaxed">{{ selectedItem.description }}</p>
            </div>
            ...
            <div class="flex space-x-3 pt-4">
              ...
              >
                <HeartIcon 
                  class="w-5 h-5" 
                  :class="selectedItem.isFavorite ? 'text-red-500 fill-current' : 'text-gray-400'"
                />
              </button>
            </div>
          </div>
