<template>
  <div :class="['flex h-screen transition-colors duration-300']">
    ...
    <main class="flex-1 p-4 md:p-6 overflow-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
        <div v-for="stat in stats" :key="stat.title" :class="['p-4 md:p-6 rounded-xl border transition-all duration-200 hover:shadow-lg cursor-pointer',
          ...
          <div class="flex items-center justify-between mb-4">
            <h3 :class="['text-sm font-medium',isDarkMode ? 'text-gray-400' : 'text-gray-600']">{{ stat.title }}</h3>
            <button :class="['hover:text-white transition-colors',isDarkMode ? 'text-gray-400' : 'text-gray-600']">⋯</button>
          </div>
          <div class="mb-4">
            <div class="text-xl md:text-2xl font-bold mb-2">{{ stat.value }}</div>
            <div class="flex items-center space-x-2">
                <span :class="['text-sm font-medium',stat.positive ? 'text-green-400' : 'text-red-400']">{{ stat.change }}</span>
              <span :class="['text-sm',isDarkMode ? 'text-gray-400' : 'text-gray-600']">{{ stat.changeValue }}</span>
            </div>
          </div>
          <button :class="['text-sm flex items-center space-x-1 transition-colors hover:text-purple-400',isDarkMode ? 'text-gray-400' : 'text-gray-600']">
      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
        ...
        <div :class="['xl:col-span-2 p-4 md:p-6 rounded-xl border',
          ...
          <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 space-y-4 md:space-y-0">
            ...
            <select v-model="selectedPeriod" @change="updateRevenue" :class="['p-2 rounded-lg border transition-colors',isDarkMode? 'bg-gray-700 text-white border-gray-600': 'bg-white text-gray-900 border-gray-300']">
          ...
          <div class="flex items-center space-x-6 mb-6">
            ...
            <div class="flex items-center space-x-2">
              ...
              <span :class="['text-sm',isDarkMode ? 'text-gray-400' : 'text-gray-600']">Loss</span>
            </div>
          </div>
          <!-- Interactive Chart -->
          <div class="h-48 md:h-64 flex items-end space-x-1 md:space-x-2">
            <div v-for="(value, index) in chartData.profit" :key="index" class="flex flex-col items-center space-y-1 flex-1 group cursor-pointer"
              ...
              <!-- Bar Container (align both bars to bottom) -->
              <div class="flex space-x-1 w-full h-full items-end relative">
                <!-- Profit Bar -->
                <div class="bg-blue-500 rounded-sm flex-1 transition-all duration-300 hover:bg-blue-400"
                    :class="{ 'transform scale-105': hoveredBar === index }" :style="{ height: (chartData.profit[index] / 20000 * 160) + 'px' }">
                  <!-- Tooltip -->
                  <div v-if="hoveredBar === index" class="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-2 py-1 rounded text-xs whitespace-nowrap z-10">
                    Profit: ${{ chartData.profit[index].toLocaleString() }}
                  </div>
                </div>
                <!-- Loss Bar -->
                <div class="bg-purple-500 rounded-sm flex-1 transition-all duration-300 hover:bg-purple-400"
                    :class="{ 'transform scale-105': hoveredBar === index }" :style="{ height: (chartData.loss[index] / 20000 * 160) + 'px' }">
                  <!-- Tooltip -->
                  <div v-if="hoveredBar === index" class="absolute -top-8 right-0 bg-gray-900 text-white px-2 py-1 rounded text-xs whitespace-nowrap z-10">
                    Loss: ${{ chartData.loss[index].toLocaleString() }}
                  </div>
                </div>
              </div>
              <!-- Label -->
              <span :class="['text-xs transition-colors',hoveredBar === index ? 'text-purple-500 font-sm': isDarkMode ? 'text-gray-400': 'text-gray-600']">{{ chartData.labels[index] }}
              </span>
            </div>
          </div>
        </div>
        <!-- Traffic Channel -->
        <div :class="['p-4 md:p-6 rounded-xl border',isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200']">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-lg font-semibold">Traffic Channel</h3>
            <select :class="['p-2 rounded-lg border text-sm transition-colors',isDarkMode? 'bg-gray-700 text-white border-gray-600': 'bg-white text-gray-900 border-gray-300']">
          ...
          <div class="flex items-center justify-center mb-6">
            <div class="relative w-32 h-32">
              <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                ...
                <circle cx="50" cy="50" r="35" fill="none" stroke="#8B5CF6" stroke-width="8" stroke-dasharray="110 220" stroke-linecap="round" class="animate-pulse"/>
                <circle cx="50" cy="50" r="35" fill="none" stroke="#A78BFA" stroke-width="8" stroke-dasharray="42 220" stroke-linecap="round" transform="rotate(158 50 50)"/>
                <circle cx="50" cy="50" r="35" fill="none" stroke="#9CA3AF" stroke-width="8" stroke-dasharray="68 220" stroke-linecap="round" transform="rotate(228 50 50)"/>
              </svg>
      ...
      <!-- Recent Activity -->
      <div :class="['mt-6 md:mt-8 rounded-xl border overflow-hidden',isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200']">
        <div :class="['p-4 md:p-6 border-b flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0',
          ...
          <div class="flex items-center space-x-2">
            <select v-model="activityFilter" @change="filterActivity" :class="['p-2 rounded-lg border text-sm transition-colors',isDarkMode
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody>
            <tr v-for="activity in filteredActivity" :key="activity.customerId" :class="['border-b transition-colors hover:cursor-pointer',
               isDarkMode? 'border-gray-700 hover:bg-gray-700': 'border-gray-200 hover:bg-gray-50']" @click="viewCustomer(activity)">
              <td class="p-3 md:p-4">
                <div class="flex items-center space-x-3">
                  ...
                  <div class="min-w-0">
                    ...
                    <div :class="['text-sm truncate',isDarkMode ? 'text-gray-400' : 'text-gray-600']">{{ activity.email }}</div>
                  </div>
                </div>
              </td>
              <td class="p-3 md:p-4">
                <span :class="['px-2 py-1 rounded-full text-xs font-medium',getStatusColor(activity.status) ]">{{ activity.status }}</span>
              </td>
              <td :class="['p-3 md:p-4 hidden md:table-cell font-mono text-sm', isDarkMode ? 'text-gray-300' : 'text-gray-700']">{{ activity.customerId }}</td>
              <td :class="['p-3 md:p-4 text-sm',isDarkMode ? 'text-gray-400' : 'text-gray-600']">{{ activity.time }}</td>
...
const chartData = ref({
  ...
  profit: [8000, 12000, 7000, 9000, 15000, 13000,8000, 12000, 6000, 9000, 15000, 13000],
  loss: [6000, 8000, 6000, 7000, 12000, 9000,6000, 12000, 6000, 9000, 15000, 13000]
})
